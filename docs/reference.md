...
- Display contract: related rows are grouped into complete release families before impact filtering. A family containing any high-impact row is shown in full by default, while the persisted `Medium impact` checkbox-style header toggle reveals the already embedded families made solely of medium-impact rows. Statistical values that have not arrived display em dashes; policy commentary and other non-statistical events leave those value cells blank.
