...
- Published selection and display contract: `stories[]` requires 9 cards for fail-open complete coverage and accepts at most 18; `crypto.notes[]` requires 9 for fail-open complete coverage and accepts at most 15; `futuresModule.stories[]` remains exactly 3 for complete coverage and accepts no extras. The editorial selection targets are 18 General and 15 Crypto; the 9/9 publication minimums are not editorial stopping points. Array order owns editorial priority. The first 9 General and first 9 Crypto cards render initially, while later accepted cards render behind a right-aligned `More stories` button in the section header. The button is absent when no secondary cards survive Apply. Secondary cards use the same published card shape and freshness, provenance, duplicate, source-fidelity, and editorial-quality rules as primary cards. A General or Crypto card receives a `New` badge when its canonical URL-first identity is absent from the dashboard published before the current masthead edition began. Masthead edition and date together identify that edition. A repeated edition preserves `previousPublishedStoryIds` and refreshes `currentPublishedStoryIds`; a different edition rotates both sets, whether Apply is scheduled or manual. Only scheduled Apply advances scheduler timestamps and completion markers.
...
- Reuters acquisition contract: Reuters' public News sitemap is the sole Reuters acquisition path. Prepare reads only the fixed HTTPS `www.reuters.com/arc/outboundfeeds/news-sitemap-index/` index and its fixed `www.reuters.com/arc/outboundfeeds/news-sitemap/` slices, requires unique contiguous 100-row offsets, and explicitly requests 100 rows so those offsets do not skip entries. The index and each slice must contain a nonempty, structurally complete set of sitemap or URL elements, and every accepted slice must yield at least one usable English Reuters article; malformed, empty, or wholly unusable documents fail that index or slice instead of passing as a successful empty acquisition. Each valid English entry enters the ordinary News pipeline with `loc` as its Reuters URL, `news:title` as its title, and the canonical URL's terminal ISO date as `publishedOn`. That URL date controls General-News freshness for both downloaded and prior-card Reuters candidates. The sitemap's `news:publication_date` becomes verified `publishedAt` only when its Chicago date agrees with the URL date. When they disagree, the URL date controls freshness and the sitemap timestamp is omitted rather than allowing a later modification to refresh an older article; conflicting stored precision is likewise omitted from a prior card. The candidate cannot enter an exact Futures window unless article-page metadata subsequently verifies its publication time. Reuters candidates then use the shared normalization, freshness, pool classification, deduplication, enrichment, and Futures-window behavior. Invalid, unsupported-locale, legacy `/default/`, malformed, or stale entries are omitted individually when another valid entry survives. Reuters candidates with a consistent verified timestamp bypass redundant article-page enrichment; conflicting candidates remain eligible for bounded article-page review when their URL date is fresh. Failed slices preserve valid entries from other slices and record the Reuters attempt as partial; an unavailable or malformed index omits Reuters candidates without degrading other acquisition paths.
...
- Display contract: related rows are grouped into complete release families before impact filtering. A family containing any high-impact row is shown in full by default, while the persisted `Medium impact` checkbox-style header toggle reveals the already embedded families made solely of medium-impact rows. Every standalone data row retains its `agency · period` subtitle; known long-form agencies use conventional display-only abbreviations without changing the stored agency or grouped-release identity. Statistical values that have not arrived display em dashes; policy commentary and other non-statistical events leave those value cells blank.
