...
- Published selection and display contract: `stories[]` requires 9 cards for fail-open complete coverage and accepts at most 18; `crypto.notes[]` requires 6 for fail-open complete coverage and accepts at most 12; `futuresModule.stories[]` remains exactly 3 for complete coverage and accepts no extras. The editorial selection targets are 18 General and 12 Crypto; the 9/6 publication minimums are not editorial stopping points. Array order owns editorial priority. The first 9 General and first 6 Crypto cards render initially, while later accepted cards render behind a right-aligned `More stories` button in the section header. The button is absent when no secondary cards survive Apply. Secondary cards use the same published card shape and freshness, provenance, duplicate, source-fidelity, and editorial-quality rules as primary cards.
- Provenance and freshness contract: downloaded URLs map directly to the approved source catalog; Prepare does not infer or replace publisher identity from hosted-page metadata. Yahoo-hosted URLs therefore remain Yahoo Finance candidates. Their `publishedAt` is hosted syndication time, recorded as `dateSource: "hosted_syndication"` and used provisionally for general-News freshness and ordering; it never sets `publishedAtVerified` or qualifies for an exact Futures timestamp window. Other first-party article pages may verify their own publication timestamp and context, but article review does not promote a candidate to another publisher or URL. `publishedAt` is optional precision and is retained only when it is an offset-bearing ISO timestamp; malformed optional values are omitted from the affected general-News card without discarding that card or unrelated candidates. Still-fresh prior cards never manufacture `publishedAtVerified` from a previously published `publishedAt` value.
...
- Yahoo currently exposes `^MOVE` as sparse latest bars rather than dependable multi-year history. A valid sparse MOVE refresh therefore merges those latest bars into the prior validated canonical MOVE history within the requested range. Valid historical OHLC bars remain unchanged; when Yahoo supplies only the latest close, that date is stored as a temporary flat candle (`open = high = low = close`) while the merged series remains OHLC. That placeholder is accepted only for strict Yahoo `MOVE`/`^MOVE` provenance with a valid `dailyChangeReference`, and the Tape information tooltip discloses that open/high/low are unavailable for the latest date. A later accepted Yahoo bar for the same date replaces the placeholder, including when real OHLC becomes available. The visible daily change uses the same Yahoo response's `meta.chartPreviousClose`, stored as `dailyChangeReference`; the reference must match the latest bar date and Yahoo provenance. A malformed prior `dailyChangeReference` is not carried forward, but the prior MOVE bars may still be used as merge history when they otherwise validate. Without both a valid fresh reference and prior validated history, the sparse refresh fails and follows the normal carried-forward/unavailable contract.
...
- Tape and chart series, including U.S. indices and equities, international and sector ETFs, commodity futures, rates-volatility and bond proxies, index futures, and crypto majors: Yahoo Finance chart data through the configured `sourceSymbol`. MOVE uses Yahoo's current `^MOVE` bar set and previous-close metadata with the validated-history merge described above; this source requires no API credential.
