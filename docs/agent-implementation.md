...
Changes to README [Update continuity and recovery](../README.md#update-continuity-and-recovery) require an observed fresh-context execution exercise as well as the downstream compatibility tests. A correct written next-action answer alone does not pass this exercise.

Use `scripts/test_context_recovery.js` to create a separate fixture for each trigger: `compaction`, `model-switch`, and `interruption`. For example, `node scripts/test_context_recovery.js setup /private/tmp/recovery-compaction compaction` creates a new isolated fixture and refuses to overwrite an existing one. Its copied policies are the current repository files. Fixtures contain synthetic inventory, saved editorial work, deliberately lagging or missing notes, and recorded command state.

Give each fresh agent only the fixture path, the trigger, and these test boundaries: resume one unfinished News review and save it; use the recorder's `read`, `poll`, and `save` commands for all fixture access; start at the fixture's `AGENTS.md`; do not inspect checker inputs, run setup/verification, access live dashboard data, research externally, or execute production workflows. Explain the recorder interface: `save ROOT JSON_PAYLOAD` replaces the entire fixture handoff and does not merge a partial patch. The recorder logs actions without enforcing the correct sequence. The agent must obtain the sequence from the copied project instructions. A parent then runs `node scripts/test_context_recovery.js verify <fixture-path>` and inspects the action trace and actual saved evidence.

The execution passes only when observed reads follow the policy routing before handoff access, inventory and saved command state are checked before saving, an active command is resolved where applicable, the first unfinished review is actually appended, prior evidence and copy remain intact, and current notes identify the next unfinished item. The interruption case polls a simulated command result; it never launches a real Prepare. Review the agent's tool history for any access outside the recorder or attempted workflow commands; such a run fails even if its final fixture passes the checker.

The checker self-tests run with the aggregate suite and reject missing or reordered prerequisites, unresolved command state, identity mismatches, repeated work, and overwritten saved work. They test the checker, not future agent behavior. Keep the three fresh-agent traces and verification results with the development report. Report this as an isolated execution exercise, not proof of an actual platform compaction or guaranteed future compliance.
