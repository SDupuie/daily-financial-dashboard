...
- For a documentation-only correction that changes no executable data contract, parser, validator, fallback, renderer, or state transition, mark the malformed-input corpus and runtime behavior classes as not applicable and verify the affected wording and command inventory instead. Documentation that introduces or changes executable behavior is not covered by this exemption.
