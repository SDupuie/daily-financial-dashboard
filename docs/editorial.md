...
- Reuters candidates downloaded through MSN syndication remain Reuters-sourced and use the reader-facing MSN article page. Treat MSN `publishedDateTime` as provider-verified publication time only after the Reuters provider identity, matching feed/detail timestamp, NewsML identity, and provider detail body validate; never substitute MSN `createdDateTime` or `updatedDateTime`. The handoff exposes the validated body only as bounded `candidate.article.excerpt`.
