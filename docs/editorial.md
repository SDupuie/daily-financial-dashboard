...
Use `editorialReview.newsSearch` as read-only source material; do not edit, delete, reorder, prune, summarize, or mark candidates unavailable. Prepare limits each finished general, Futures, and Crypto inventory to the 250 newest eligible deduplicated candidates; every emitted candidate remains part of the required editorial review.
