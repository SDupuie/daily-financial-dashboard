...
For metadata triage, run this read-only `jq` query from the repository root. It prints 100 URL-deduplicated candidates with their pool memberships and review fields. Repeat it with `start` set to 100, 200, and so on until a batch is empty; inspect every batch. The query writes only to the terminal. Use full records from the same candidate file for deep review. Do not create a secondary inventory file, helper script, or alternate projection command.

```sh
jq -c --argjson start 0 --argjson size 100 '
  ([.generalCandidates[] | {bucket:"General", item:.}]
   + [.futuresCandidates[] | {bucket:"Futures", item:.}]
   + [.cryptoCandidates[] | {bucket:"Crypto", item:.}])
  | group_by(.item.url)
  | .[$start:($start+$size)][]
  | {pools:(map(.bucket)|unique), title:.[0].item.title,
     sourceLabel:.[0].item.sourceLabel, publishedOn:.[0].item.publishedOn,
     publishedAt:.[0].item.publishedAt,
     publishedAtVerified:.[0].item.publishedAtVerified,
     priorCard:any(.[]; .item.priorCard == true), url:.[0].item.url}
' generated/news_candidates.json
```
...
3. Review downloaded News after prices and before making any editorial decisions.
   - Complete metadata triage under the News inventory contract before writing card copy or beginning another editorial section.
   - Deeply review every retained contender and expand review when required by the News inventory and News-card contracts. Working triage notes are allowed; they do not edit the handoff or establish a fixed shortlist ceiling.
