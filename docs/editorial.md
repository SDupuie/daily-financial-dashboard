...
3. Review downloaded News after prices and before making any editorial decisions.
   ...
   - Complete the News inventory contract's metadata-triage pass over the URL-deduplicated union of General, Futures, and Crypto before editing Opening, Futures stories, Tape commentary, Moving Today, Crypto notes, Earnings narrative, Week Ahead commentary, or Market Lens. Build a first-pass contender set targeting 90 unique General/Futures candidates and 60 unique Crypto candidates. Retain the five strongest qualifying unique candidates from each represented General/Futures `sourceLabel` and the three strongest qualifying unique candidates from each represented Crypto `sourceLabel`; retain every qualifying candidate when a source has fewer than its floor and return unused places to the applicable overall pool. When a pool is smaller than its target, carry the entire pool forward. Include Futures-only candidates and every still-fresh prior card; do not stop after finding enough stories to fill a section.
   ...
   - After triage is complete, deeply review every retained contender under the News inventory contract. Inspect available summaries, descriptions, `candidate.article.excerpt`, and, when needed, the reader-facing page before ranking or writing. Do not silently skip a candidate because article-page context is missing, and do not select a candidate using metadata alone.
   ...
   - Treat every candidate in the generated News inventory as eligible for metadata triage. Rank only deeply reviewed contenders. Fill the primary News-card slots first, including the qualified Marketaux single-ticker priority described above, then continue adding strong, distinct secondary cards until the 18 General and 15 Crypto targets are reached. If either section remains below target, expand deep review until every eligible candidate in that pool has been exhausted; do not invent filler or lower the selection standards to reach a target.
