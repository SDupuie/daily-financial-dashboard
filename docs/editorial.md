...
Maintain `generated/editorial/review-progress.md` as the single plain-text checkpoint for resuming AI review. The AI creates and updates it after Prepare; no updater, Apply, validator, or published page reads it. Initialize a fresh checkpoint only after `generated/news_candidates.json` `generatedAt` matches the handoff's `editorialReview.preparedAt`. Replace the prior checkpoint for each new inventory. Use this format:

- Record both matching timestamps and the scan order: `generalCandidates` in array order, then `futuresCandidates` entries whose URL was not already seen, then `cryptoCandidates` entries whose URL was not already seen. A repeated URL is one candidate even if it belongs to two pools.
- Record completed contiguous metadata-scan ranges as 1-based positions in that URL-deduplicated order, the inventory reference of the last inspected candidate, and the next unread position. Mark a range complete only after all its metadata was actually inspected; retrieve truncated output before advancing.
- Record each retained contender once, using its first inventory reference in scan order: a zero-based array path such as `generalCandidates[12]`, `futuresCandidates[3]`, or `cryptoCandidates[8]`. Mark it `pending` or `reviewed`; a `reviewed` entry needs a brief evidence note and decision based on its full record. Resolve pool membership from the inventory URL when needed; do not type it into the checkpoint.
- Record the status of other editorial sections, unresolved questions, and one concrete next action. The checkpoint is a current-state record: replace superseded entries and totals instead of appending corrections that leave contradictory claims.

For example, after inspecting the first candidate, write `Metadata: 1-1 complete; last=generalCandidates[0]; next=2`. A contender entry can be `generalCandidates[0] | pending` and later `generalCandidates[0] | reviewed | retain: distinct rate-policy angle supported by excerpt`. These are examples of format, not claims about the current inventory.

Copy inventory references from indexed read-only inventory output and confirm each reference resolves before recording it. Never type, reconstruct, shorten, or correct a candidate URL in the checkpoint; keep URLs, article text, copied inventory records, and long contender narratives out of it. Derive contender counts from the distinct resolved references after triage rather than manually maintaining running totals. Copy the exact `candidate.url` from the inventory into the handoff when making a News selection; the checkpoint is never a source for selection URLs or article facts.

Update the checkpoint after each completed metadata batch, meaningful deep-review group, and completed editorial section. Record only work actually completed. After context compaction or interruption during AI Editorial Work, compare both checkpoint timestamps with the current inventory and handoff, resolve every referenced index against that inventory, and check that ranges are contiguous and statuses have the required evidence notes. Resume from the next verified unfinished step. Treat stale timestamps, unresolved or duplicate references, contradictory ranges or statuses, and unsupported completion claims as unverified; use saved outputs and the handoff to recover only supported progress, then review the affected work. Do not spend the update repairing URL aliases or preserving an inconsistent checkpoint. Compaction alone does not require restarting Prepare or repeating verified review. After successful Apply, the checkpoint is closed and has no role in the published update; the next Prepare requires a fresh checkpoint for its inventory.
