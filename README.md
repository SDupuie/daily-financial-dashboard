...
- **Publish:** finalization validates the complete dashboard and atomically replaces the canonical dashboard.

### Opening editorial decision contract

Opening is optional at publication. A populated `headline` controls the headline/deck block: if `headline` is missing or blank, the dashboard omits that block even when `deck` is present. Catalyst chips render independently, and only catalyst items with both `label` and `body` render. Missing or incomplete Opening fields are Stage 2 editorial guidance, not final publication blockers.
...
4. Editorialize the generated handoff in this order.
   ...
   - `opening`: when supportable, rewrite `headline`, `deck`, and catalyst items for the current edition. A missing or blank headline omits the headline/deck block; catalyst chips render independently only when both `label` and `body` are populated.
...
| --- | --- | --- |
| `stories[]` | Target 9 broad-market cards from the generated News inventory | `tag`, `title`, `body`, HTTPS `url`, `publishedOn` |
| `crypto.notes[]` | Target 6 crypto-specific cards from the generated News inventory | `kicker`, `title`, `body`, HTTPS `url`, `publishedOn` |
| `futuresModule.stories[]` | Target 3 current catalysts from the generated News inventory | `tag`, `title`, `body`, HTTPS `url`, `publishedOn`, offset-bearing ISO `publishedAt` |
...
- `publishedOn` is an `America/Chicago` date in `YYYY-MM-DD` format. Stage 1 owns freshness filtering; final validation verifies date shape.
- Futures `publishedAt` must be a verified offset-bearing ISO timestamp. Stage 1 owns Futures session-window eligibility.
- Finalization mechanically dedupes selected duplicate URLs by keeping the first selected card and omitting later duplicates. Duplicate titles, section category, and below-target counts are editorial/Stage-1 concerns rather than publication blockers.
...
For a chart-only correction, start with a current complete candidate. Use `--apply-chart-data-json` for a complete chart payload, `--merge-chart-data-json` for selected series, or `--sync-chart-quotes` for derived quote views. Then regenerate the editorial handoff and use normal finalization; successful quote changes require reviewed commentary. If the candidate is missing or stale, rerun deterministic preparation.
...
- For quick iteration or an ordinary non-publish check, run `node scripts/validate_dashboard.js daily_financial_news.html`. It enforces date shape, Week Ahead and Earnings lifecycle/reaction coherence, embedded-data text hygiene, quote/data binding, and the runtime's LAN-only refresh endpoint contract. It is not an additional step after content-only readiness or the complete suite.
...
- Avoid market-superlative claims unless directly verified during Stage 2. `verifiedClaims[]` may record optional supporting evidence, but unsupported superlative language is editorial guidance rather than a publication-blocking validator gate.
...
- `opening`: optional current-edition market-open summary. `headline` controls whether the headline/deck block renders; `catalysts[]` items render independently when both `label` and `body` are populated.
...
- `note` should explain the factor driving that ticker or market today. Avoid restating quote values or including source/citation language.
...
Use `node scripts/earnings_week.js validate` or `node scripts/earnings_week.js validate-release` for standalone Earnings validation.
