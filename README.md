...
4. Search news after prices.
  ...
  - A carried-forward link may stay only if it remains one of the best available stories after that fresh comparison. If two candidates are materially similar, prefer the newer publication date unless the older piece is substantially stronger by reporting quality and price-relevance.
   ...
   - Before finalizing story links, perform an accessibility substitute check for any subscriber, metered, or commonly gated source.
   - When a clearly accessible, reputable source supports the same claim with similar timeliness and specificity, use that source instead. Use subscriber, metered, or commonly gated sources such as Axios, Barron's, MarketWatch, Bloomberg, WSJ, FT, and similar outlets only when they are original, materially stronger, or no suitable accessible substitute exists.
   - Preferred general market/news sources include AP, readable Reuters, CNBC, Investopedia, Kiplinger, Investor's Business Daily, Yahoo Finance, Morningstar, TheStreet, U.S. News Money, and official exchange or index-provider pages.
   - Preferred primary/official sources include company investor-relations releases, SEC filings, Federal Reserve, Treasury.gov, BLS, BEA, EIA, CME/NYMEX, Nasdaq, NYSE, Cboe, S&P Dow Jones Indices, MSCI, and ETF issuer pages such as iShares, Vanguard, State Street, Fidelity, and Invesco.
   - Preferred crypto sources include CoinDesk, Decrypt, Blockworks, CoinGecko, CoinMarketCap, Alternative.me, issuer pages for listed crypto ETFs, SEC filings, and official protocol, exchange, or company announcements.
