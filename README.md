...
The scheduled preflight is read-only: it enforces the weekday/Chicago-time window and refuses a completed `YYYY-MM-DD:morning|afternoon` marker. The final scheduled apply enforces that guard again; manual preparation and application bypass it. Every deterministic refresh assembles and validates `generated/daily_financial_news.candidate.html` with refreshed futures, chart data, Tape quote fields, crypto stat cards, Asset Allocation data, Week Ahead values and lifecycle, earnings results, the Chicago run date, edition/session labels, the compile timestamp, and the session portion of `tape.label`; it does not alter `daily_financial_news.html`. A staging candidate may contain pending Earnings or Week Ahead editorial work. Publication requires each applicable field to have either verified copy or an explicit unavailable disposition; unavailable editorial evidence does not block deterministic facts or reactions. Week Ahead calendar slates normally rebuild only on Friday afternoon and Monday morning, but release values refresh every run without moving official dates or times. Friday afternoon shows the current Friday plus next Monday-Thursday, while Monday morning replaces that Friday with the current Monday-Friday slate. If a rollover produces a correctly dated unavailable Week Ahead or Earnings shell, every subsequent run retries that same displayed range until a validated slate replaces the shell. Validation refuses to complete an active scheduled edition unless both embedded calendar ranges match the exact slate required for that date and window. `--prepare-editorial-dir` copies `dashboard-data` from the staged complete candidate into the editorial workspace, removes any stale receipt and legacy keys, creates `earnings_narrative.json` bound to the staged Earnings facts, and creates a review skeleton bound to the unchanged canonical `baseEditionId`, required sections, and every event date. The final apply command rebuilds from the deterministic candidate and copies only the documented editorial surfaces from `dashboard-data.json`: Opening copy, News/Futures/Crypto stories, Tape roster and commentary, Week Ahead outcomes, and Tape/footer contextual suffixes. If that candidate is missing, malformed, or stale, finalization starts from the validated canonical dashboard, records the fallback, and retries preparation. It applies Earnings narrative only to the deterministic staged Earnings rows, supplies `reviewedAt`, advances the scheduled News baseline when requested, derives final quote views from staged chart data, applies every Market Lens decision, finalizes missing post-event copy as retryable unavailable dispositions, stamps the edition and receipt in a temporary final candidate, validates it, and only then atomically replaces the published artifact.

A source-refresh failure is isolated at the narrowest canonical boundary during deterministic preparation. Earnings result refresh isolates provider and ticker attempts, Futures isolates contracts, Chart/Tape and Asset Allocation isolate tickers, Crypto isolates stat providers, and Week Ahead isolates schedule authorities and indicator requests. Successful items publish together with same-period retained values or item-level unavailable shells and explicit partial diagnostics. The Asset Allocation portfolio and sanitized summary are separate transactions, so either success survives failure of the other. A whole-section command/artifact failure still uses the validated same-range carry or correctly dated unavailable shell; a new Asset Allocation month and Week Ahead/Earnings rollover never reuse stale dates. Malformed optional editorial items are dropped or replaced by the validated candidate value, malformed Earnings narrative input becomes retryable unavailable dispositions, and unusable Market Lens decisions use the generated domain default. Every degraded state is retried on the next run and clears after recovery. No source, staging, evidence, or editorial-data failure may stop publication: the updater must first select and validate the applicable fallback. Validation remains strict about the selected completed artifact; only the absence of any validated base artifact or an inability to write, atomically replace, or validate the completed artifact can prevent publication.
...
Earnings editorial completion gate: the common editorial pass must research every staged narrative task, preserve evidence-backed pre-release commentary through `released_awaiting_close`, and set `editorialAttempted: true` on each researched row in `generated/editorial/earnings_narrative.json`. If the pass is missing or unusable, finalization records the retryable unavailable disposition itself; it does not block the dashboard or leave a blank Outcome column. `commentary_unavailable` and `unverified` remain publishable only as explicit dispositions and are reconsidered on the next run.
...
The update architecture is staged and transactional. The canonical section-by-section ownership and write-authority matrix lives in `AGENTS.md`; this README owns the corresponding operational runbook and data contracts. Producers atomically write deterministic artifacts under `generated/`, reject individual source items they cannot reliably normalize, and retain accepted neighboring items with explicit partial diagnostics. Staging artifacts that persist across runs, anchor sidecars or later decisions, support editorial work, or pass through multiple domain commands receive explicit domain validation before downstream consumption; simple one-hop payloads do not require separate validators. A successful command is accepted only after the updater reads and validates the exact artifact it wrote. One preparation owns `generated/daily_update.lock` at a time. The updater assembles every deterministic change into `generated/daily_financial_news.candidate.html`, validates that complete staging candidate, and leaves the canonical dashboard untouched. Final editorial application verifies that the staged candidate still matches the canonical base edition, validates the finalized candidate with its editorial receipt, and atomically replaces the canonical dashboard. Each successful canonical replacement refreshes ignored `generated/daily_financial_news.last_good.html`; malformed canonical JSON may use that validated snapshot as its assembly base, but the recovered candidate still passes normal final validation before replacement. Focused repair modes use the same candidate-validation and atomic-replacement function but are explicit transactions rather than the normal three-phase path. Public workflow CLIs are `run_daily_update.js`, `validate_dashboard.js`, `earnings_week.js`, `local_market_server.js`, and `publish_main.sh`; most `fetch_*.js` commands are public staging producers. `fetch_chart_data.js` is the consolidated public Chart/Futures producer and core: it owns source retrieval, the pure Futures staging-payload contract, and pure chart rounding, compaction, and quote-row projection shared by its callers. The updater invokes that Futures contract but owns only application and cross-section labels; `validate_dashboard.js` independently validates the final embedded Futures section. Files such as `earnings_week_build.js` and `earnings_week_validation.js` are private implementations and must be reached through `earnings_week.js`.
...
| `staging_writer.js` | Private filesystem helper used by staging producers, the Earnings core/build implementation, and the updater | File path plus serialized or JSON payload in; atomically replaced staging/cache file out | Staging/cache and ignored recovery files only; never dashboard HTML |
...
- `./scripts/publish_main.sh` runs the complete readiness validation, including the full regression suite, before it pushes. A missing or stale editorial receipt is rebuilt from the validated candidate with its system-applied fallbacks before publication; a receipt that still cannot validate indicates a completed-artifact integrity failure.
...
- Run `node scripts/validate_dashboard.js test` after script or data-contract changes. It checks tracked JavaScript and shell syntax, the LaunchAgent plist, all four default test routines, the canonical embedded dashboard contracts, HTML structure, and whitespace errors. It is self-contained and does not require ignored `generated/` artifacts or network access. The default batch intentionally excludes LAN/local-refresh integration coverage; run `node scripts/test_dashboard.js --local-refresh` when changing the local market server, its TLS/origin policy, or browser-side local-refresh behavior.
...
- A Chart/Tape source failure may carry the complete last validated series only with matching `chart-data.availability` and `tape.availability` objects; quote rows and visible Tape values are still re-derived from that carried canonical series.
...
1. The orchestrator builds a calendar slate when a Friday-afternoon or Monday-morning target range differs from the current staged `generated/earnings_week.json` range, when the embedded target range is still an explicitly unavailable rollover shell, or when a display-eligible row remains `primary_only` on a later Central-time day. Friday uses current Friday plus next Monday-Thursday; Monday uses the current Monday-Friday. This permits one bounded daily corroboration retry without repeating the paid calendar scan during development or same-day manual repairs. Run `node scripts/earnings_week.js build --from YYYY-MM-DD --to YYYY-MM-DD` only for an authorized production rollover or explicit manual rebuild.
...
9. Compare active-week EarningsAPI-discovered symbols against the admitted Finnhub slate. Queue only symbols absent from Finnhub as `secondaryRecoveryCandidates`; same-symbol date conflicts must collapse to one audited canonical row, not duplicate recovery rows.
