...
| --- | --- | --- | --- |
| Scheduled | Run `node scripts/run_daily_update.js --scheduled --morning` or `node scripts/run_daily_update.js --scheduled --afternoon` to enforce the scheduler-only start/duplicate guard and stage `generated/daily_financial_news.candidate.html`, then run `node scripts/run_daily_update.js --prepare-editorial-dir generated/editorial --morning` or `--afternoon`; that command downloads and deduplicates the News candidates before writing the handoff | Edit the single `generated/editorial/dashboard-data.json` handoff. Complete every non-News `pending_review` item and null decision, review the complete downloaded News pool, and select and write the strongest available stories. | Run `node scripts/run_daily_update.js --apply-dashboard-data-json generated/editorial/dashboard-data.json --scheduled --morning` or `--afternoon`; this rechecks the completion marker, merges only allowed editorial fields, derives News coverage from the valid selected cards, and atomically replaces the canonical dashboard, after which commit on `main` and run `./scripts/publish_main.sh` |
...
- **Fail open:** deterministic source failures do not take the update offline. Source fallbacks keep preparation moving and may preserve coherent same-period data or correctly dated unavailable states. Required editorial work has no partial, exhausted, or unavailable completion state. If required editorial work or a valid complete candidate cannot be assembled, finalization stops and the existing dashboard remains online.
...
The handoff starts `editorialReview.openingDecision.action` at `null`. After reviewing the complete Opening section, set it to `reviewed`. Any other action, a missing decision, or malformed Opening copy cannot finalize.
...
4. Editorialize the generated handoff in this order.
   ...
   - `opening`: review the complete headline, deck, and four catalysts, then update `headline`, `deck`, and four concise catalyst items. Set `editorialReview.openingDecision.action` to `reviewed` only after that review is complete; no other action finalizes.
...
- `opening`: market-open summary with `headline`, `deck`, and exactly four `catalysts[]` items. The generated editorial handoff requires `editorialReview.openingDecision.action: "reviewed"`; no unavailable action finalizes.
