...
- Before editing, run the scheduled preflight for the active window. Finalization with `--scheduled` repeats the weekday/time and embedded completion-marker checks before writing. A completed scheduled morning run blocks another scheduled morning run, but not the afternoon run. Intentional same-window reruns must use the manual/on-demand commands without `--scheduled`; they remain allowed and preserve the scheduled News baseline.
...
| --- | --- | --- | --- |
| Scheduled | Run `node scripts/run_daily_update.js --scheduled-preflight --morning` or `--afternoon`, then run `node scripts/run_daily_update.js --morning` or `node scripts/run_daily_update.js --afternoon` to stage `generated/daily_financial_news.candidate.html`, then run `node scripts/run_daily_update.js --prepare-editorial-dir generated/editorial --morning` or `--afternoon` | Edit the single `generated/editorial/dashboard-data.json` handoff. Replace every `pending_review` item and null decision with reviewed work or an explicit current-run unavailable/partial disposition only after that attempt is exhausted. | Run `node scripts/run_daily_update.js --apply-dashboard-data-json generated/editorial/dashboard-data.json --scheduled --morning` or `--afternoon`; this merges only allowed editorial fields into the staged candidate and atomically replaces the canonical dashboard, after which commit on `main` and run `./scripts/publish_main.sh` |
...
- **Commentary:** fresh quotes require commentary bound to that quote revision. Finalization accepts reviewed copy or an explicit current-run attempted-unavailable disposition; it does not manufacture an unavailable result for unattempted work. Failed quote downloads retain their prior validated quote and commentary together.
...
- **Fail open:** this applies only during final publication after every required editorial section and row has been reviewed or explicitly attempted to exhaustion. It never ends or substitutes for editorial work. Deterministic source fallbacks keep preparation moving so review can continue; finalization may then preserve successful neighbors and publish same-period carried data or correctly dated unavailable states selected by those completed attempts. Unattempted `pending_review` work or null decisions do not finalize. If no valid replacement can be assembled, the existing dashboard remains online.
...
The handoff also starts `editorialReview.openingDecision.action` at `null`. After reviewing the complete Opening section, set it to `reviewed`. If Opening commentary cannot be completed after the current-run attempt is exhausted, set it to `commentary-unavailable` with a current-run `attemptedAt` and reason; finalization then uses the neutral generated Opening fallback. A missing decision or malformed Opening marked `reviewed` cannot finalize. System fallback handling never changes this decision or marks Opening complete.

### Market Lens editorial decision contract

The single dashboard-data handoff contains the draft receipt in `editorialReview`. Its `marketLensDecisions[]` must contain one decision for every current event day. Scheduled or awaiting events use `replace` or `retain-generated`. Once any event is released, pre-release copy is invalidated and the decision must use current `replace` commentary or `commentary-unavailable` with a current-run `attemptedAt` and reason. The unavailable action publishes neutral visible fallback copy without hiding released facts.
...
The AI fills the Opening decision, each generated event-day decision, and any verified-claim evidence; the script supplies `reviewedAt`, `baseEditionId`, and event dates. Application embeds the reviewed base edition, decision summary, verified-claim evidence, new edition ID, and payload hash. Each Tape row carries the canonical chart-series `quoteRevision` and either a reviewed or `commentary_unavailable` note disposition. After the relevant editorial completion gate is satisfied, application may quarantine malformed or unsupported optional input and record the system-applied `retained_candidate`, `omitted`, `generated_default`, or `unavailable_disposition` action. These system actions are not human verification, cannot complete editorial work, and are removed after a later valid editorial apply. Every later payload rewrite clears or invalidates the receipt, and a manifest cannot be reused after the base edition changes.
...
2. Run the normal deterministic refresh before reading news.
   ...
   - Earnings facts are refreshed into the deterministic candidate even when new narrative is required. Unchanged deterministic rows retain their reviewed narrative; a facts or lifecycle change invalidates only that row. The single handoff marks every incomplete or previously unavailable required field `pending_review`. Supply verified replacement copy when supportable, or replace it with a field-specific current-run `commentary_unavailable`/`unverified` disposition after the attempt is exhausted. Finalization rejects remaining pending work and never converts neighboring rows or the entire section to unavailable.
   ...
   - Review the complete `opening` headline, deck, and four catalysts. Set `editorialReview.openingDecision.action` to `reviewed` only after that review is complete. After an exhausted attempt, use `commentary-unavailable` with current-run `attemptedAt` and reason; do not use fail-open behavior as a reason to stop Opening research or leave the decision null.
   - In each `tape.rows[].note` whose quote was successfully downloaded, summarize the relevant market commentary or catalyst driving that market. Do not carry its prior commentary forward or restate `last`, `delta`, or `pct`. The handoff marks that row `pending_review`. If supportable commentary cannot be completed after the attempt, explicitly set `commentary_unavailable` with a current-run `attemptedAt` and reason. A failed quote row retains its last validated quote and bound commentary unchanged.
...
4. Editorialize the generated handoff in this order.
   ...
   - After the deterministic Week Ahead refresh, evaluate all releases on each event day against the current Tape, opening, and verified news. Scheduled or awaiting event days may `retain-generated` or use a valid `replace` lens. Once any event is released, generic pre-release copy is no longer eligible: supply current `replace` commentary or an explicit current-run `commentary-unavailable` disposition. At `close_available`, retain the preselected reaction ticker set and also supply either a verified editorial `outcome` or its explicit retryable unavailable disposition. Do not alter calendar facts, restate displayed values, use source/process language, or write tactical-allocation advice.
...
- The editorial handoff starts `storiesCoverage`, `crypto.notesCoverage`, and `futuresModule.storiesCoverage` at `pending_review`. After the current-run search-and-winnow pass, set each to `status: "complete"` with `checkedAt` when its target is met, or `status: "partial"`, `reason: "insufficient_qualifying_fresh_coverage"`, and `checkedAt` when the attempt is exhausted below target. Finalization derives the published coverage state from the accepted collections. Partial sections remain eligible for replenishment on every later update. This is audit metadata only and must never render as inline section copy or footer content.
...
- `opening`: market-open summary with `headline`, `deck`, and exactly four `catalysts[]` items. The generated editorial handoff also requires `editorialReview.openingDecision`: `reviewed` for a completed Opening review, or `commentary-unavailable` with current-run `attemptedAt` and reason after an exhausted attempt. The decision is a handoff gate; an unavailable decision produces a neutral generated Opening fallback and a receipt system-fallback record.
...
The apply command requires exactly one decision for every current event day. The manifest has no `sections` declaration or all-sections editorial gate. A malformed decision, stale event ID, missing evidence reference, noncanonical reaction ticker, missing chart, or unverified superlative leaves editorial finalization incomplete and the canonical dashboard unchanged. Correct that item in the same handoff. When current interpretation was genuinely attempted but remains unavailable, record the explicit current-run `commentary-unavailable` decision; scheduled or awaiting events may still retain the validated generated lens.
...
- A successfully downloaded quote invalidates the prior row note before editorial handoff. Missing, invalid, or repeated prior copy remains `pending_review` and cannot finalize. After a real current-run attempt, an explicit retryable unavailable disposition is valid for publication; stale commentary is never a fallback for a fresh quote. A failed quote download retains the last validated quote and its bound commentary.
...
9. During the common editorial phase, edit only narrative fields and their dispositions inside `generated/editorial/dashboard-data.json`, using the verified numeric row plus official-release evidence. AI must not invent data, dates, estimates, actuals, reaction values, business drivers, or guidance. Pre-release `outcome.interpretation` is required for every display-eligible row; changed deterministic facts invalidate only the affected row. At `close_available`, required fields are `outcome.interpretation`, `outcome.guide` or an official no-guidance/current-run unverified disposition, and `reaction.note`. Verified copy retains the existing length and substance checks. An exhausted attempt uses the field-specific unavailable disposition with no unsupported text, a reason, and current-run `attemptedAt`.
10. The normal final apply reads those narrative fields from the single handoff, matches them to immutable candidate rows by symbol/date, and requires reviewed copy or an explicit attempted-unavailable disposition for every applicable field. Remaining `pending_review`, malformed, stale, or contradictory content leaves the canonical dashboard unchanged. Accepted unavailable rows publish individually and remain retryable; they never convert successful neighboring rows or the whole Earnings section.
