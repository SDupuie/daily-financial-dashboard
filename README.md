...
3. Review downloaded News after prices.
   - `--prepare-editorial-dir` automatically runs `scripts/fetch_news_candidates.js` before it writes `dashboard-data.json`. The worker executes every fixed base and fallback query in `scripts/news_sources.js` against the dated GDELT DOC article-search endpoint, filters results through the checked-in approved-source catalog, downloads each accepted reader-facing article page when accessible, normalizes freshness and HTTPS URLs, deduplicates canonical URLs, and records every query's counts or error in `editorialReview.newsSearch`.
...
- For an ad hoc News acquisition diagnostic, use `node scripts/fetch_news_candidates.js --input daily_financial_news.html --as-of TIMESTAMP --morning|--afternoon --output generated/news_candidates.json`. It requires no API key, executes the checked-in GDELT query manifest, adds still-fresh cards from the input dashboard, and writes staging only. The normal workflow invokes it automatically from `--prepare-editorial-dir`.
