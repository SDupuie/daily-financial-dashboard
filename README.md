...
- **Fail open:** deterministic source failures and News shortages do not take the update offline. Source fallbacks keep preparation moving and may preserve coherent same-period data or correctly dated unavailable states. News may publish with any number of valid cards, including zero, while required non-News editorial work still has no partial, exhausted, or unavailable completion state. If required non-News editorial work or a valid complete candidate cannot be assembled, finalization stops and the existing dashboard remains online.
...
2. Run the normal deterministic refresh before reading news.
   ...
   - Earnings facts are refreshed into the deterministic candidate even when new narrative is required. Every visible row needs a reviewed pre-release thesis before results. A row remains `pending` or `awaiting_actual` when its release window has passed but reliable actuals are not yet available; retain that thesis and do not invent results. Once actual results exist, replace the pre-release thesis with verified interpretation, supported guidance treatment, and verified reaction commentary when the closing response is available. Finalization rejects missing visible-row commentary.
   ...
   - Review the complete `opening` headline, deck, and four catalysts. Set `editorialReview.openingDecision.action` to `reviewed` only after that review is complete; no other action finalizes.
   - In each `tape.rows[].note` whose quote was successfully downloaded, summarize the relevant market commentary or catalyst driving that market. Do not carry its prior commentary forward or restate `last`, `delta`, or `pct`. The handoff marks that row `pending_review`, and finalization requires newly reviewed copy. A failed quote row retains its last validated quote and bound commentary unchanged.
...
3. Review downloaded News after prices.
   ...
   - Alpha Vantage requires `ALPHA_VANTAGE_API_KEY`; StockFit requires `STOCKFIT_API_KEY`. The two Alpha calls are paced. Every provider and feed is fetched fresh on every run; a failed path does not stop any other path or publication.
   ...
   - JavaScript performs acquisition only. It does not rank, select, diversify, or write stories. The AI reviews the entire downloaded pool plus every still-fresh prior card and owns relevance, source quality, angle diversity, final selection, and reader-facing copy. Article-page failures remain visible on the individual candidate but do not discard the search result or stop another path.
   - The acquisition goals are at least 36 eligible general-market candidates for the combined general/Futures review and at least 12 eligible Crypto candidates. These are diagnostics, not publication gates and do not stop the fixed acquisition manifest early. If every configured path has been attempted and the pool is still short, review everything available and continue.
   - Compare downloaded candidates with still-fresh prior cards directly; retain a prior card when it remains among the strongest relevant, source-faithful coverage, and do not discard or churn it merely because the scheduled window changed.
   - Verify the selected article's title, publication date, accessible HTTPS reader-facing URL, source fidelity, and duplication before writing its card. Futures selections also require an offset-bearing `publishedAt` inside the active session window.
   - Aim for 9 broad-market cards, 4–6 Crypto cards, and 3 Futures catalysts. If any target remains short, keep every strong valid card and continue to finalization; the updater derives partial coverage and News never blocks an otherwise valid publication.
...
| --- | --- | --- |
| `stories[]` | Target 9 broad-market, non-crypto cards; 0–9 may publish | `tag`, `title`, `body`, HTTPS `url`, `publishedOn` |
| `crypto.notes[]` | Target 4–6 crypto-specific cards; 0–6 may publish | `kicker`, `title`, `body`, HTTPS `url`, `publishedOn` |
| `futuresModule.stories[]` | Target 3 current catalysts; 0–3 may publish | `tag` (24 characters or fewer to preserve the shared label column), `title`, `body`, HTTPS `url`, `publishedOn`, offset-bearing ISO `publishedAt` |

- `editorialReview.newsSearch` is generated by the downloader and contains its complete path-attempt log plus all mechanically eligible downloaded and still-fresh prior `generalCandidates[]` and `cryptoCandidates[]`. The pool goals are 36 general and 12 Crypto candidates, but missing candidates, failed paths or article-page downloads, and manual valid additions never block finalization. Candidate pools remain only in the generated handoff and are not embedded in the publication receipt.
- The updater owns `storiesCoverage`, `crypto.notesCoverage`, and `futuresModule.storiesCoverage`. It writes `status: "complete"` when the target is met or `status: "partial"`, `reason: "insufficient_qualifying_fresh_coverage"`, and `checkedAt` when it is short, including zero cards. The editorial handoff cannot set the published disposition.
- Partial coverage relaxes only the minimum count. Maximum counts, required fields, HTTPS URLs, freshness, duplicate detection, New-pill identity, Futures timing, and claim verification remain strict for every published card.
...
- `stories`: zero to nine broad-market, non-crypto news cards plus updater-owned `storiesCoverage`. See the News-card contract.
...
- `crypto`: crypto section metadata, crypto-only stat rows, zero to six Crypto story notes, and updater-owned `notesCoverage`. Crypto ticker quote rows do not live here.
...
The richer earnings monitor uses this contract as the canonical deterministic method. The production dashboard consumes the canonical earnings week payload from embedded `dashboard-data.earnings.week`; provider sidecars are build-time inputs only. AI commentary must describe the current deterministic stage: pre-release preview commentary is required for each display-eligible scheduled or awaiting-actual row unless a current-run editorial attempt records an explicit unavailable fallback. An arrived result changes the deterministic fingerprint and invalidates that row's pre-release copy. Once a result or close response is available, current post-release interpretation, guidance disposition, and reaction commentary are required as applicable unless current official evidence or research supports a field-specific unavailable or unverified disposition.
...
9. During the common editorial phase, edit only narrative fields and their dispositions inside `generated/editorial/dashboard-data.json`, using the verified numeric row plus official-release evidence. AI must not invent data, dates, estimates, actuals, reaction values, business drivers, or guidance. Every visible row needs a reviewed pre-release `outcome.interpretation`; a row whose release window has passed but whose actuals remain unavailable stays deterministically `pending` or `awaiting_actual` and retains that thesis. Once results exist, replace it with a result interpretation; at `close_available`, also provide `outcome.guide` or an official evidenced `not_provided` disposition and `reaction.note`. Verified copy retains the existing length and substance checks.
