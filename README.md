...
Earnings editorial completion gate: the common editorial pass must research every staged narrative task, preserve evidence-backed pre-release commentary through `released_awaiting_close`, and set `editorialAttempted: true` on each researched row in `generated/editorial/earnings_narrative.json`. Finalization rejects a skipped narrative pass; `commentary_unavailable` or `unverified` is publishable only when the editorial attempt is explicitly recorded. This prevents a deterministic Earnings refresh from silently becoming a blank Outcome column.
...
- `run_daily_update.js` derives `storiesCoverage`, `crypto.notesCoverage`, and `futuresModule.storiesCoverage` from the finalized collections. Normal counts use `status: "complete"`; a shortage uses `status: "partial"`, `reason: "insufficient_qualifying_fresh_coverage"`, and a retry timestamp. Partial sections remain eligible for replenishment on every later update. This is audit metadata and must not render as inline section copy or footer content; it may appear only through an explicitly requested information tooltip.
