...
| --- | --- | --- | --- |
| Scheduled | Run `node scripts/run_daily_update.js --scheduled --morning` or `node scripts/run_daily_update.js --scheduled --afternoon` to enforce the scheduler-only start/duplicate guard and stage `generated/daily_financial_news.candidate.html`, then run `node scripts/run_daily_update.js --prepare-editorial-dir generated/editorial --morning` or `--afternoon`; that command downloads and deduplicates the News candidates before writing the handoff | Edit the single `generated/editorial/dashboard-data.json` handoff. Complete every non-News `pending_review` item and null decision. For News, follow the News-card contract and write selected cards only to `futuresModule.stories`, `stories`, and `crypto.notes`. | Run `node scripts/run_daily_update.js --apply-dashboard-data-json generated/editorial/dashboard-data.json --scheduled --morning` or `--afternoon`; this rechecks the completion marker, merges only allowed editorial fields, validates against the News-card contract, and atomically replaces the canonical dashboard, after which commit on `main` and run `./scripts/publish_main.sh` |
| Manual/on-demand | Run `node scripts/run_daily_update.js --morning` or `node scripts/run_daily_update.js --afternoon`, then run `node scripts/run_daily_update.js --prepare-editorial-dir generated/editorial --morning` or `--afternoon` | Complete the same single generated handoff | Run the matching apply command without `--scheduled`; commit and publish only when the manual update is intended to go live |
...
- **Commentary:** fresh quotes require reviewed commentary bound to that quote revision. Failed quote downloads retain their prior validated quote and commentary together.
- **Publish:** finalization stamps the receipt, validates the complete candidate, atomically replaces the dashboard, and refreshes the last-good snapshot. Focused repairs update the current complete candidate and use this same finalization path.
...
Opening must be freshly rewritten for the current edition. Rewrite `headline`, `deck`, and all four `catalysts[]` items before setting `editorialReview.openingDecision.action` to `"reviewed"`. Unchanged carried-forward Opening copy is not reviewed editorial work.

### Market Lens editorial decision contract

The single dashboard-data handoff contains the draft receipt in `editorialReview`. Its `marketLensDecisions[]` must contain one decision for every current event day. Scheduled or awaiting events use `replace` or `retain-generated`. Once any event is released, pre-release copy is invalidated and the decision must use current `replace` commentary.
...
The AI fills the Opening decision, each generated event-day decision, and any verified-claim evidence; the script supplies `reviewedAt`, `baseEditionId`, and event dates. Application embeds the reviewed base edition, decision summary, verified-claim evidence, new edition ID, and payload hash. Each refreshed Tape row carries reviewed commentary bound to the canonical chart-series `quoteRevision`; a failed quote refresh retains its prior quote-and-commentary bundle. After the relevant editorial completion gate is satisfied, application may omit malformed unsupported optional input, but that system action never completes editorial work. Every later payload rewrite clears or invalidates the receipt, and a manifest cannot be reused after the base edition changes.
...
3. Review downloaded News after prices and before making any editorial decisions.
   - `--prepare-editorial-dir` automatically runs `scripts/fetch_news_candidates.js` before it writes `dashboard-data.json`. The worker executes the complete fixed acquisition manifest in `scripts/news_sources.js`, filters results through the checked-in approved-source catalog, downloads reader-facing article pages when accessible, normalizes freshness and HTTPS URLs, deduplicates canonical URLs and titles, and records every path's counts or error in `editorialReview.newsSearch`; see the News-card contract for source and selection requirements.
   - `editorialReview.newsSearch` is Stage 2 source material, not editorial output. Read it completely, but do not edit, delete, reorder, prune, summarize, or mark candidates unavailable. The only News fields the AI edits are `futuresModule.stories`, `stories`, and `crypto.notes`.
   - After eligibility filtering and deduplication, Stage 1 keeps the most recent 250 downloaded candidates for article-page review and editorial handoff; older overflow candidates are intentionally discarded to keep acquisition bounded.
   - Alpha Vantage requires `ALPHA_VANTAGE_API_KEY`; StockFit requires `STOCKFIT_API_KEY`. Every provider and feed is attempted fresh on every run; a failed path does not stop any other path. If News acquisition times out after staging valid reviewed candidates, the updater continues with that partial candidate set plus still-fresh prior cards.
   ...
   - Treat every candidate in the generated News inventory as eligible for editorial consideration. Before making any editorial decision, review the complete inventory and rank the candidates. Apply the required selection counts in the News-card contract exactly; when the generated inventory for a collection contains fewer candidates than required, select every available candidate. Do not reduce a section below its required selection because a candidate is weaker, narrower, or less prominent than another candidate.
   - Follow the News-card contract and Story selection policy below for required fields, counts, freshness, source choice, carry-forward decisions, and link rules.

4. Editorialize the generated handoff in this order.
   - `masthead`: leave the generated edition and date unchanged.
   - `opening`: freshly rewrite `headline`, `deck`, and all four catalyst items for the current edition. Do not preserve prior Opening copy, and do not set `editorialReview.openingDecision.action` to `reviewed` until all Opening fields are current and complete.
...
| Collection | Required selection | Required fields |
| --- | --- | --- |
| `stories[]` | Select exactly 9 broad-market, non-crypto cards from the remaining generated general inventory, or every generated candidate if fewer than 9 exist | `tag`, `title`, `body`, HTTPS `url`, `publishedOn` |
| `crypto.notes[]` | Select exactly 6 crypto-specific cards, or every generated Crypto candidate if fewer than 6 exist | `kicker`, `title`, `body`, HTTPS `url`, `publishedOn` |
| `futuresModule.stories[]` | Select exactly 3 current catalysts from Futures-window-eligible generated candidates, or every eligible candidate if fewer than 3 exist | `tag` (24 characters or fewer to preserve the shared label column), `title`, `body`, HTTPS `url`, `publishedOn`, offset-bearing ISO `publishedAt` |

- `editorialReview.newsSearch` is generated by the downloader and contains its complete path-attempt log plus all mechanically eligible downloaded and still-fresh prior `generalCandidates[]` and `cryptoCandidates[]`. It is read-only reference material for Stage 2; selected article cards belong only in `futuresModule.stories`, `stories`, and `crypto.notes`. A selected card that is not drawn from the generated candidate inventory is not valid editorial output. The pool goals are 36 general and 12 Crypto candidates. Candidate pools remain only in the generated handoff and are not embedded in the publication receipt.
...
- Finalization verifies selected cards against the generated News candidate artifact and this News-card contract. The editable `editorialReview.newsSearch` copy is not a publication source, and finalization never selects replacement stories.
...
- `editorialReview`: publication receipt with `schemaVersion`, `reviewedAt`, `reviewedBaseEditionId`, matching `reviewedEditionId`, one summarized `marketLensDecisions[]` entry per event day, optional `verifiedClaims[]`, optional system-owned sanitation records, and a SHA-256 `payloadHash`. The hash covers the complete embedded `dashboard-data` payload except the receipt itself plus the complete embedded `chart-data` payload. Any direct or orchestrated data/chart rewrite therefore invalidates the receipt; only an explicit editorial apply command with a matching one-time `baseEditionId` creates it. The candidate pools in the generated handoff are deliberately omitted from this publication receipt.
...
- `opening`: current-edition market-open summary with `headline`, `deck`, and four `catalysts[]` items.
...
The apply command requires exactly one decision for every current event day. The manifest has no `sections` declaration or all-sections editorial gate. A malformed decision, stale event ID, missing evidence reference, noncanonical reaction ticker, missing chart, unverified superlative, or unavailable decision leaves editorial finalization incomplete and the canonical dashboard unchanged. Scheduled or awaiting events may retain the validated generated lens; released events require current replacement commentary.

### `weekAhead.days[].outcome`

`outcome` replaces the visible forward-looking Market Lens only when the day reaches `close_available`. It requires `status: "verified"`, editorial `title` and `body`, and `source: "editorial"`. The copy should synthesize all released facts on that day, distinguish the actual-versus-forecast result from the session response, and avoid asserting exclusive causality. Deterministic reactions remain derived from the preselected Market Lens tickers and store each ticker's event-day and previous-trading-day closes, delta, percent change, unit, direction, role, and chart date. The renderer omits flat rows and orders visible movers by absolute movement; it does not select unrelated Tape winners after the fact, and post-close decisions cannot replace the preselected ticker set. A changed actual, forecast, previous value, provenance field, or closing bar invalidates the prior outcome and requires new verified editorial work.
...
- `noteDisposition` binds the note to the accepted quote revision. `status: "reviewed"` requires newly reviewed copy and `reviewedAt`, and `noteDisposition.quoteRevision` must exactly equal the matching canonical `chart-data.series[].quoteRevision`; the visible quote fields remain derived from that same series.
- A successfully downloaded quote invalidates the prior row note before editorial handoff. Missing, invalid, unavailable, or repeated prior copy remains `pending_review` and cannot finalize. A failed quote download retains the last validated quote and its bound commentary.
...
- A partial Chart/Tape source failure may carry the complete last validated series only when the failed ticker appears exactly once in `chart-data.availability.failures[]` and that same series has `availability.status: "carried_forward"`; every carried series must have that matching failure entry. `chart-data` is the sole availability owner, and the Tape renders its warning directly from that metadata. Quote rows and visible Tape values are still re-derived from the mixed canonical series, and the last quote-bound commentary remains unchanged for failed tickers. Only successfully downloaded quotes reset their Tape commentary binding, and they require newly reviewed copy before publication.
...
9. During the common editorial phase, edit only narrative fields and their dispositions inside `generated/editorial/dashboard-data.json`, using the verified numeric row and the evidence required by the Earnings editorial contract. Complete every visible row for its current narrative state and transition. AI must not invent data, dates, estimates, actuals, reaction values, business drivers, or guidance. Verified copy retains the existing length and substance checks.
10. The normal final apply reads those narrative fields from the single handoff, matches them to immutable candidate rows by symbol/date, and requires verified copy for every applicable released-result field. Remaining `pending_review`, unavailable, malformed, stale, or contradictory editorial content leaves the canonical dashboard unchanged.
11. For an explicit Earnings-only repair, first complete deterministic preparation so the current complete candidate exists, then use `node scripts/earnings_week.js apply-narrative` and run `node scripts/run_daily_update.js --apply-earnings-week-json generated/earnings_week.json` to update that candidate. Continue with the normal `--prepare-editorial-dir` and `--apply-dashboard-data-json` steps. A missing or stale candidate requires deterministic preparation; a malformed review queue is warning-only; malformed or invalid Earnings staging leaves both candidate and canonical dashboard unchanged. `earnings_week.js` never edits dashboard HTML, and the published dashboard must not fetch staging files at runtime.
...
The standalone `earnings_narrative.json` sidecar remains only for the explicit Earnings-only repair command. The normal dashboard workflow does not create or consume it as an operator handoff; all normal editorial work is contained in `generated/editorial/dashboard-data.json`.
