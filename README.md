...
- `--scheduled` identifies only a scheduler-driven preparation or finalization. Manual/on-demand and development runs omit it and may occur on any day, at any time, and more than once per edition or scheduled window.
- In `America/Chicago`, the scheduled morning start window is 7:45–9:00 AM and the scheduled afternoon start window is 3:45–5:00 PM on weekdays. Scheduled preparation checks that window and the embedded completion marker before any fetch or staging write.
- Scheduled finalization rechecks the completion marker immediately before writing. A run that passed scheduled preparation may finish after the start window closes. A completed scheduled morning run blocks another scheduler-driven morning completion, but not a manual rerun or the afternoon run.
...
| --- | --- | --- | --- |
| Scheduled | Run `node scripts/run_daily_update.js --scheduled --morning` or `node scripts/run_daily_update.js --scheduled --afternoon` to enforce the scheduler-only start/duplicate guard and stage `generated/daily_financial_news.candidate.html`, then run `node scripts/run_daily_update.js --prepare-editorial-dir generated/editorial --morning` or `--afternoon`; that command downloads and deduplicates the News candidates before writing the commentary file. Creating `generated/editorial/dashboard-data.json` seals the candidate. Any later deterministic change requires repeating preparation and regenerating the commentary file. | Edit the single `generated/editorial/dashboard-data.json` commentary file. Review the complete downloaded News pool and give every required item a terminal editorial disposition. Run `node scripts/run_daily_update.js --validate-dashboard-data-json generated/editorial/dashboard-data.json --morning` or `--afternoon`; this pure, read-only editorial check reports every error by exact path without assembling a dashboard. Correct only the reported items and rerun it under the bounded loop below. | Run `node scripts/run_daily_update.js --apply-dashboard-data-json generated/editorial/dashboard-data.json --scheduled --morning` or `--afternoon`; this rechecks the completion marker and candidate hash, reruns the same editorial input contract, applies only allowed editorial fields, derives publication metadata, validates the assembled dashboard, and atomically replaces the canonical dashboard, after which commit on `main` and run `./scripts/publish_main.sh` |
| Manual/on-demand | Run `node scripts/run_daily_update.js --morning` or `node scripts/run_daily_update.js --afternoon`, then run `node scripts/run_daily_update.js --prepare-editorial-dir generated/editorial --morning` or `--afternoon`; creating the commentary file seals the candidate under the same rule | Complete and validate the same single generated commentary file | Run the matching apply command without `--scheduled`; commit and publish only when the manual update is intended to go live |

The editorial phase uses a bounded per-item `edit -> validate` loop. Make at most three correction passes for an invalid item and stop early when its payload does not change or its error signature repeats. A required item must then be terminal: valid reviewed/verified copy, an applicable `not_provided` guidance disposition, or `dropped_after_review` with empty copy plus `attemptedAt` and `reason`. If the sealed candidate already contains commentary valid for the current revision, retain it instead of dropping it. Otherwise the deterministic facts publish without that commentary. Preparation mechanically reopens every dropped item as `pending_review` during the next update. The pass count and error signature exist only in the running editorial task; they are not persisted in another marker, ledger, or queue.
...
- **Commentary:** the single generated dashboard-data commentary file owns narrative text, decisions, and dispositions. The commentary file records the sealed candidate's payload hash, and finalization rejects it if the candidate has changed. Fresh quotes require a terminal disposition bound to that quote revision; unchanged revisions retain valid commentary unless the commentary file explicitly rewrites it. Failed quote downloads retain their prior validated quote and commentary together.
- **Publish:** finalization applies commentary, derives publication metadata, validates the assembled dashboard, and installs it. It does not refresh or recalculate prepared data. Focused repairs update the current complete candidate and use this same finalization path.
- **Fail open:** deterministic source failures do not take the update offline. Source fallbacks keep preparation moving and may preserve coherent same-period data or correctly dated unavailable states. Editorial work cannot remain `pending_review`: after bounded review it is either valid or explicitly `dropped_after_review`. Finalization stops and leaves the existing dashboard online only when the sealed inputs, terminal editorial contract, assembled artifact, or atomic installation is invalid.

The scheduler guard belongs only to commands carrying `--scheduled`. Manual and development commands bypass both the start window and scheduled completion marker. A dashboard update request covers staging and canonical generated data only; source code, tests, documentation, configuration, and policy require separate authorization.
...
After deterministic preparation has created the current complete candidate, use `--apply-chart-data-json` for an already-generated chart-only payload; it validates and embeds the series in that candidate and rebuilds matching `chart-data.quoteRows` and visible Tape prices. To add or repair only selected chart series without refreshing the rest of the Tape, fetch each with `scripts/fetch_chart_data.js --ticker SYMBOL` and stage the resulting payload with `--merge-chart-data-json`; that preserves every untouched or failed series, quote row, and bound note while invalidating commentary only for successfully downloaded merged quotes. `--sync-chart-quotes` also stages its derived quote views. All three commands require the current candidate and leave the canonical dashboard unchanged; if it is missing or stale, rerun deterministic preparation. Complete focused deterministic repairs before generating the commentary file. Afterward, commentary-only changes belong in that file. Continue with `--prepare-editorial-dir` and the normal `--apply-dashboard-data-json` finalization command; the commentary file marks refreshed ticker commentary `pending_review`, and each item must become reviewed or `dropped_after_review` before publication.
...
### Opening editorial decision contract

The handoff starts `editorialReview.openingDecision.action` at `null`. After reviewing the complete Opening section, set it to `reviewed`. If bounded correction does not converge, set it to `dropped-after-review` with `attemptedAt` and `reason`; finalization then retains the sealed candidate's valid Opening. A null, unsupported, or malformed decision cannot finalize.

### Market Lens editorial decision contract

The single dashboard-data handoff contains the draft receipt in `editorialReview`. Its `marketLensDecisions[]` must contain one decision for every current event day. Scheduled or awaiting events use `replace` or `retain-generated`. Once any event is released, pre-release copy is invalidated and the decision must use current `replace` commentary or, after bounded review, `dropped-after-review` with `attemptedAt` and `reason`.

`scripts/week_ahead_contract.js` owns calendar-payload validation, event release states, comparable actual-versus-forecast surprises, preservation of still-valid pre-close editorial lenses through `released_awaiting_close`, post-close outcome invalidation when facts or closing reactions change, event-day close-reaction derivation, and Market Lens decision coverage and action semantics. `scripts/run_daily_update.js` requires current editorial interpretation for released events and verifies cross-section references and chart closes against the assembled dashboard.
...
Before the close, the visible Market Lens remains forward-looking. Once at least one actual is available and the afternoon chart payload contains completed event-day and previous-trading-day closes for every selected transmission ticker, the day transitions to `close_available`. The renderer then replaces the forward-looking lens with verified `Outcome & Close Reaction` editorial copy interpreting the released facts and session response. Do not scan the Tape after the fact for the largest movers or imply that one release caused the entire session when several catalysts were active.

The AI fills the Opening decision, each generated event-day decision, and any verified-claim evidence; the script supplies `reviewedAt`, `baseEditionId`, and event dates. Application embeds the reviewed base edition, decision summary, verified-claim evidence, new edition ID, and payload hash. Each refreshed Tape row carries a terminal disposition bound to the canonical chart-series `quoteRevision`; a failed quote refresh retains its prior quote-and-commentary bundle. The editorial validator reports malformed or unsupported input by exact item path; finalization reruns that same contract as a safety backstop. Every later payload rewrite clears or invalidates the receipt, and a manifest cannot be reused after the base edition changes.
...
2. Run the normal deterministic refresh before reading news.
   ...
   - Earnings facts are refreshed into the deterministic candidate even when new narrative is required. Complete every visible row under the Earnings editorial contract in Appendix: Earnings operations. Do not invent results, guidance, business drivers, or reaction context.
...
3. Review downloaded News after prices and before making any editorial decisions.
   - `--prepare-editorial-dir` automatically runs `scripts/fetch_news_candidates.js` before it writes `dashboard-data.json`. The worker executes the complete fixed acquisition manifest in `scripts/news_sources.js`: separate Alpha Vantage `financial_markets` and `blockchain` calls, one 50-story StockFit market call, and direct RSS feeds for Investing.com market, economy, indicators, earnings, company, commodities, and Crypto plus Axios, Kiplinger, Investor's Business Daily, CoinDesk, Decrypt, CNBC, and MarketWatch. It filters results through the checked-in approved-source catalog, downloads each accepted reader-facing article page when accessible, normalizes freshness and HTTPS URLs, deduplicates canonical URLs and titles, and records every path's counts or error in `editorialReview.newsSearch`.
   - Alpha Vantage requires `ALPHA_VANTAGE_API_KEY`; StockFit requires `STOCKFIT_API_KEY`. The two Alpha calls are paced. Every provider and feed is fetched fresh on every run; a failed path does not stop any other path.
   - Direct-feed provenance wins when the same CNBC, MarketWatch, or Investor's Business Daily story also arrives through Alpha Vantage or StockFit. For a StockFit Yahoo Finance URL, the worker preserves the syndication metadata and uses an original-publisher URL only after an explicit URL is found on the Yahoo page, the domain is approved, the article page loads, and its title matches. Otherwise it truthfully retains the Yahoo-hosted reader URL. It never guesses an original URL from a publisher name or home page.
   - JavaScript performs acquisition only. It does not rank, select, diversify, or write stories. The AI owns relevance review, source-quality assessment, angle diversity, final selection, and reader-facing copy.
   ...
   - Review each candidate in sufficient detail to assess its title and subject, publisher and source fidelity, publication date and exact timestamp, reader-facing URL, available feed summary or article extract, article-page acquisition status, current-dashboard relevance, substantial overlap with other candidates, and eligibility for the authoritative Futures publication window. When an article page was acquired, use the available article-page detail. When it was unavailable, review the complete feed-level record and acquisition diagnostics; do not silently skip the candidate.
   ...
   - The acquisition goals are at least 36 eligible general-market candidates for the combined general/Futures review and at least 12 eligible Crypto candidates. These diagnostics do not stop the fixed acquisition manifest early. Regardless of pool size, complete the review of every available candidate.
   - Compare all downloaded candidates with every still-fresh prior card. Retain a prior card only when it remains among the strongest relevant, source-faithful coverage; do not discard or churn it merely because the scheduled window changed.
   - Treat every candidate in the generated News inventory as eligible for editorial consideration. Before making any editorial decision, review the complete inventory and rank the candidates. Select exactly 3 distinct Futures-window-eligible stories first, then 9 Moving Today stories from the remaining general inventory, and 6 Crypto stories whenever the corresponding inventory contains enough candidates. When an inventory contains fewer than its target, select every available candidate. Do not reduce a section below target because a candidate is weaker, narrower, or less prominent than another candidate.
   - Follow the News-card contract and Story selection policy below for required fields, freshness, source choice, carry-forward decisions, and link rules.

4. Editorialize the generated handoff in this order.
   - `masthead`: leave the generated edition and date unchanged.
   - `opening`: review the complete headline, deck, and four catalysts, then update `headline`, `deck`, and four concise catalyst items. Set `editorialReview.openingDecision.action` to `reviewed` after valid copy is complete, or to `dropped-after-review` with `attemptedAt` and `reason` after the bounded correction limit; the latter retains the sealed candidate's valid Opening.
   ...
   - `tape`: leave generated quote fields unchanged; update the editorial roster only when intentionally changing coverage, and review commentary for every successfully downloaded quote. Each verified `tape.rows[].note` must summarize the relevant market commentary or catalyst without carrying prior commentary forward or restating `last`, `delta`, or `pct`. Every Crypto-group ticker needs its own current note for the collapsed Tape Crypto tab; do not reuse generic copy across BTC, ETH, SOL, XRP, IBIT, ETHA, MSTR, or other visible Crypto tickers. After bounded failure, use an empty note with `noteDisposition.status: "dropped_after_review"`, the current `quoteRevision`, `attemptedAt`, and `reason`. Leave failed-download rows on their last validated quote and bound commentary.
...
| --- | --- | --- |
| `stories[]` | Operational target 9 broad-market, non-crypto cards; maximum 9 | `tag`, `title`, `body`, HTTPS `url`, `publishedOn` |
| `crypto.notes[]` | Operational target 6 crypto-specific cards; maximum 6 | `kicker`, `title`, `body`, HTTPS `url`, `publishedOn` |
| `futuresModule.stories[]` | Operational target 3 current catalysts; maximum 3 | `tag` (24 characters or fewer to preserve the shared label column), `title`, `body`, HTTPS `url`, `publishedOn`, offset-bearing ISO `publishedAt` |

- `editorialReview.newsSearch` is generated by the downloader and contains its complete path-attempt log plus all mechanically eligible downloaded and still-fresh prior `generalCandidates[]` and `cryptoCandidates[]`. The pool goals are 36 general and 12 Crypto candidates. Candidate pools remain only in the generated handoff and are not embedded in the publication receipt.
- The updater owns `storiesCoverage`, `crypto.notesCoverage`, and `futuresModule.storiesCoverage`; do not set or edit those fields in the editorial handoff.
- News cards have no separate disposition object. Correct an invalid selected card within the bounded editorial loop or omit that card from its selected array after the limit; finalization derives the resulting coverage. It does not invent or restore rejected card copy.
...
Manual fallback work never goes directly into `daily_financial_news.html`. Put the verified fallback into the appropriate staging artifact or generated editorial handoff, then return to the normal updater-owned candidate and finalization workflow. If the correct entry point is unclear, stop and check the relevant data contract rather than editing the published dashboard.

Market Lens has no separate repair or publication command. For a Market Lens-only correction, use the current complete candidate when it is still bound to the canonical `editionId`, regenerate the single `dashboard-data.json` handoff, and make the decision there. If no current candidate exists, rerun deterministic preparation first. The normal dashboard-data finalization remains the only canonical replacement path.
...
- For an ad hoc News acquisition diagnostic, use `node scripts/fetch_news_candidates.js --input daily_financial_news.html --as-of TIMESTAMP --morning|--afternoon --output generated/news_candidates.json`. It loads `ALPHA_VANTAGE_API_KEY` and `STOCKFIT_API_KEY` from `.env` or the environment, executes the complete checked-in API/feed manifest, adds still-fresh cards from the input dashboard, and writes staging only. The normal workflow invokes it automatically from `--prepare-editorial-dir`.
- Individual staging fetchers are `node scripts/fetch_chart_data.js`, `node scripts/fetch_crypto_stats.js`, `node scripts/fetch_news_candidates.js`, `node scripts/fetch_asset_allocation.js`, and `node scripts/fetch_week_ahead.js`; use `node scripts/fetch_chart_data.js futures --session` for afternoon Session Futures.
...
After readiness and the Git push succeed, `publish_main.sh` waits for the matching GitHub Pages deployment and verifies that the live page contains the expected `editionId`. For a recognized transient Pages failure or stale deployment, it may create and push one empty retry commit by default, then verifies the retried deployment. This publication recovery changes Git history only; it never changes dashboard data.
...
- `editorialReview`: publication receipt with `schemaVersion`, `reviewedAt`, `reviewedBaseEditionId`, matching `reviewedEditionId`, one summarized `marketLensDecisions[]` entry per event day, optional `verifiedClaims[]`, and a SHA-256 `payloadHash`. The generated commentary file also carries `candidatePayloadHash`, which binds it to the complete sealed candidate and is retained in the publication receipt; a candidate-hash mismatch rejects the handoff. The publication hash covers the complete embedded `dashboard-data` payload except the receipt itself plus the complete embedded `chart-data` payload. Any direct or orchestrated data/chart rewrite therefore invalidates the receipt; only an explicit editorial apply command with a matching one-time `baseEditionId` creates it. The candidate pools in the generated commentary file are deliberately omitted from this publication receipt.
...
- `opening`: market-open summary with `headline`, `deck`, and exactly four `catalysts[]` items. The generated editorial handoff requires `editorialReview.openingDecision.action: "reviewed"` or terminal `"dropped-after-review"` with `attemptedAt` and `reason`; a dropped decision retains the sealed candidate's valid Opening.
...
- `stories`: broad-market, non-crypto news cards with a maximum of nine, plus updater-owned `storiesCoverage`. See the News-card contract.
- `newsBaseline`: embedded scheduled-update comparison state for the News Flow and Crypto `New` pills. `currentScheduledStoryIds` stores the most recent scheduled run's `stories[]` and `crypto.notes[]` identities, while `previousScheduledStoryIds` is the comparison set used to keep manual runs from consuming scheduled newness. `lastScheduledWindow` is the completed `YYYY-MM-DD:morning|afternoon` marker checked by the next scheduled preparation and finalization.
...
- `crypto`: crypto section metadata, crypto-only stat rows, Crypto story notes with a maximum of six, and updater-owned `notesCoverage`. Crypto ticker quote rows do not live here.
...
The apply command requires exactly one terminal decision for every current event day. The manifest has no `sections` declaration or all-sections editorial gate. The editorial validator reports every malformed decision, stale event ID, missing evidence reference, noncanonical reaction ticker, missing chart, and unverified superlative by exact path. Scheduled or awaiting events may retain the validated generated lens. Released events require current replacement commentary or `dropped-after-review`; a dropped decision retains valid candidate commentary for the revision when available, otherwise it publishes the deterministic transmission map without lens prose.

### `weekAhead.days[].outcome`

`outcome` replaces the visible forward-looking Market Lens only when the day reaches `close_available`. It requires either `status: "verified"`, editorial `title` and `body`, and `source: "editorial"`, or `status`/`source: "dropped_after_review"`, empty prose, `attemptedAt`, and `reason`. Verified copy should synthesize all released facts on that day, distinguish the actual-versus-forecast result from the session response, and avoid asserting exclusive causality. Deterministic reactions remain derived from the preselected Market Lens tickers and store each ticker's event-day and previous-trading-day closes, delta, percent change, unit, direction, role, and chart date. The renderer omits flat rows and orders visible movers by absolute movement; it does not select unrelated Tape winners after the fact, and post-close decisions cannot replace the preselected ticker set. A changed actual, forecast, previous value, provenance field, or closing bar invalidates the prior outcome; the next preparation reopens verified or dropped commentary for the changed revision.
...
- `noteDisposition` binds the note to the accepted quote revision. `status: "reviewed"` requires newly reviewed copy and `reviewedAt`; `status: "dropped_after_review"` requires an empty note, `attemptedAt`, and `reason`. In both cases `noteDisposition.quoteRevision` must exactly equal the matching canonical `chart-data.series[].quoteRevision`; the visible quote fields remain derived from that same series.
- A successfully downloaded quote invalidates the prior row note before editorial handoff. Missing, invalid, or repeated prior copy remains `pending_review` and cannot finalize. After bounded review it may become `dropped_after_review` and publish without a note. A failed quote download retains the last validated quote and its bound commentary. The next preparation reopens every dropped row even when its quote revision is unchanged.
- A Tape row whose chart series is carried forward, or whose latest bar trails the dashboard reference date by at least two business dates, shows the same 16 px stale-data info marker beside its ticker symbol. A later successful current-date refresh clears the marker automatically.
...
- `crypto.stats[]` is for crypto-only section stat cards: Fear & Greed, Altcoin Season Index, and Crypto Market Cap. A failed or explicitly skipped refresh retains each last validated card with row-level `availability.status: "carried_forward"` and preserves its successful timestamp in `availability.lastValidatedAt`; the UI then places a 16 px stale-data info marker beside that card title with the validation time in Central time. An unavailable row renders one `Unavailable` value without a subtitle, change, suffix, or visualization. When no validated cards exist, the explicit unavailable fallback contains an empty array.
...
- A partial Chart/Tape source failure may carry the complete last validated series only when the failed ticker appears exactly once in `chart-data.availability.failures[]` and that same series has `availability.status: "carried_forward"`; every carried series must have that matching failure entry. `chart-data` is the sole availability owner, and the Tape renders its warning directly from that metadata. Quote rows and visible Tape values are still re-derived from the mixed canonical series, and the last quote-bound commentary remains unchanged for failed tickers. Only successfully downloaded quotes reset their Tape commentary binding and require a reviewed or `dropped_after_review` disposition.
...
The richer earnings monitor uses this contract as the canonical deterministic method. The production dashboard consumes the canonical earnings week payload from embedded `dashboard-data.earnings.week`; provider sidecars are build-time inputs only.
...
1. **Calendar rollover:** Perform a fresh pre-release review for every visible row in the new slate, including companies retained from the preceding calendar. Prior copy cannot be carried forward as completed work.
...
Every required Earnings narrative field must receive company-specific review and reach a terminal disposition. Valid commentary uses `verified`; confirmed absence of guidance may use `not_provided`. Generic, duplicated, templated, or unsupported text must not be marked verified. After bounded correction fails, use `dropped_after_review` with empty copy, `attemptedAt`, and `reason`; deterministic facts remain visible and the next preparation reopens that field as `pending_review`.

### Source hierarchy

1. Finnhub primary: calendar slate, company profile, market cap, timing, EPS estimate, EPS actual, revenue estimate, and revenue actual when Finnhub has the row. Display eligibility uses an exact symbol match in the cached Finnhub U.S. stock-symbol directory and rejects OTC MICs; profile headquarters country and a profile-returned foreign primary ticker do not override that listing evidence.
...
Allowed `reportTiming` values are `bmo`, `amc`, `dmh`, and `unknown`. Allowed lifecycle values are `scheduled`, `awaiting_actual`, `released_awaiting_close`, and `close_available`. Allowed metric `result` values are `beat`, `miss`, `met`, `not_compared`, and `pending`. Allowed `outcome.overall` values are `beat`, `miss`, `mixed`, `met`, `eps_only_beat`, `eps_only_miss`, `pending`, and `unverified`. Allowed `reaction.basis` values are `same_day_close`, `next_session_close`, `during_market_close`, and `unavailable`. Allowed `reaction.status` values are `pending`, `awaiting_close`, `computed`, and `unavailable`; `unavailable` is reserved for a reported row whose reaction window cannot be resolved, not a close that simply has not happened yet. Allowed `sourceSummary.primary` values are `finnhub`, `earningsApiCompany`, and `sec_company_release`. The dashboard consumes this canonical row contract directly and derives display labels, tones, and compact metric text at render time. The source artifact keeps field-level selected sources in `sourceAudit`; dashboard-visible provenance uses compact `sourceSummary` plus `sourceStatus`. The existing company-row information indicator covers retained and unconfirmed data without exposing provider identity: a carried-forward same-week slate shows `Last validated earnings data: [date/time] CT.`, an uncorroborated `primary_only` or `secondary_only` schedule shows `Report date is unconfirmed.`, and a row meeting both conditions combines both sentences in the same tooltip. Healthy `corroborated` and `official_confirmed` rows have no company indicator. Retained slates have no section-wide warning; if no current-week slate exists, the normal Earnings shell displays only `Unavailable`, while the reason remains in embedded metadata and logs. Every applicable narrative field must be terminal: verified company-specific copy, `not_provided` guidance, or `dropped_after_review` with empty copy and attempt metadata. A pending field blocks finalization; a dropped field publishes the deterministic row without that prose and reopens during the next preparation. An unresolved reaction window displays only `Unavailable`, without explanatory operational subtext. Days without covered rows display `No covered earnings scheduled.` Audit snapshots preserve provider identity by section name, but metric values still use canonical `eps` and `revenue` objects rather than provider field names.
...
- During an authorized weekly build, query EarningsAPI calendar from seven calendar days before the displayed range through 14 calendar days after it. Use only the five displayed trading dates for secondary discovery. This 26-date scan is authorized only by scheduler-marked Monday-morning and Friday-afternoon rollovers, an audited failed-rollover or unavailable-shell retry on a later scheduler-marked run, or `--rebuild-earnings-calendar` on an intentional manual preparation. It never runs during ordinary manual updates, development, validation, tests, source-code verification, focused repairs, or result refreshes.
...
- `scripts/earnings_week_contract.js` owns the shared earnings row policy, narrative-completion and unchanged-row carry policy, staged-slate rebuild decisions, schedule-review applicability, fail-open row dispositions, row-level result-refresh status, and same-range/rollover section fallback. It also owns Finnhub calendar-field normalization, EarningsAPI daily-bucket and request/outcome audit mechanics, Yahoo reaction rules, and company-release task construction. Result collection isolates Finnhub, EarningsAPI company, and Yahoo attempts; a required-call or unreadable-ledger failure is recorded only on affected rows and does not discard independent successes. `run_daily_update.js` invokes the contract-owned fallback only when the resulting Earnings artifact itself cannot be validated and reads normal Earnings commentary from the single dashboard-data handoff.
- `scripts/earnings_week.js` is the only public Earnings CLI and module owner. It owns refresh, resolution, and apply commands and coordinates the staging files used by the contract decisions imported by the updater. `scripts/earnings_week_build.js` is its private build implementation and must not be invoked directly. A malformed persisted Earnings artifact does not itself authorize the metered build: the updater rebuilds only when the run also has scheduled rollover/retry authority or an explicit manual rebuild; otherwise it retains the validated active-range section through the documented fallback.
...
1. Scheduler-marked Friday-afternoon and Monday-morning preparation builds the rollover slate when its target range differs from staging. Friday uses current Friday plus next Monday-Thursday; Monday uses the current Monday-Friday. Remaining `primary_only` rows do not authorize another Finnhub/EarningsAPI calendar build after a successful scan. An audited failed scan or unavailable rollover shell may retry on a later scheduler-marked run under the existing Central-time usage ledger. Ordinary manual preparation retains and refreshes the validated active range even on Monday or Friday; use `node scripts/run_daily_update.js --rebuild-earnings-calendar --morning|--afternoon` only when deliberately authorizing a manual metered rebuild.
2. Fetch Finnhub earnings calendar for the selected five trading days.
3. Fetch Finnhub profiles for Finnhub rows and filter display eligibility by an exact cached Finnhub U.S. stock-symbol match, non-OTC MIC, market cap, profile quality, and watchlist rules. Treat profile country as issuer domicile, not listing eligibility.
...
7. On every scheduled dashboard run, work the active schedule-review queue without rebuilding the calendar slate after a successful rollover scan. For each active `primary_only` row, retry company IR first and SEC/EDGAR only if IR does not resolve the schedule date. A successful confirmation must be recorded against the active `symbol` and Finnhub `primaryDate`; stale, out-of-range, or prior-quarter confirmation rows do not satisfy the queue. Rows that remain unresolved stay `primary_only`, publish as partial, and remain eligible for official-source retry on the next scheduled run while they remain in the active range. The only same-range provider-calendar exception is a later-day retry of an audited rollover attempt that was incomplete, skipped, errored or throttled, or returned no rows.
...
1. Run `node scripts/earnings_week.js refresh` against the existing staged `generated/earnings_week.json`; outside scheduler-marked Friday-afternoon/Monday-morning rollover preparation, do not rebuild a valid slate. If the embedded current range is an explicitly unavailable rollover shell, retry that same range only on later scheduler-marked runs (or an explicit manual rebuild) until it validates.
...
7. Compute EPS and revenue results mechanically from numeric estimate and actual values. When estimate and actual render equally at the dashboard's metric precision—two decimals for EPS and the displayed B/M/dollar precision for revenue—classify the result as `met`; otherwise classify it as `beat` or `miss`. If revenue estimate is unavailable, produce an EPS-only outcome and mark revenue `not_compared`.
...
9. During the common editorial phase, edit only narrative fields and their dispositions inside `generated/editorial/dashboard-data.json`, using the verified numeric row and the evidence required by the Earnings editorial contract. Review every visible row for its current narrative state and transition. AI must not invent data, dates, estimates, actuals, reaction values, business drivers, or guidance. Verified copy retains the existing length and substance checks. For an invalid field, correct and validate it at most three times; stop early on no change or a repeated error signature, then use an empty `dropped_after_review` disposition with `attemptedAt` and `reason`.
10. The normal final apply reads those narrative fields from the single handoff, matches them to immutable candidate rows by symbol/date, and requires a terminal disposition for every applicable field. Remaining `pending_review`, malformed, stale, or contradictory editorial content leaves the canonical dashboard unchanged. A valid `dropped_after_review` field does not block the row or dashboard; it publishes no prose and is mechanically reopened during the next preparation.
11. For an explicit Earnings-only repair, first complete deterministic preparation so the current complete candidate exists, then run the normal `--prepare-editorial-dir` command and edit every applicable Earnings narrative field directly in the single `generated/editorial/dashboard-data.json` handoff. Do not route commentary through `earnings_week.json` or `--apply-earnings-week-json`; that focused command stages deterministic Earnings facts only. Finish with the normal `--apply-dashboard-data-json` finalization. A missing or stale candidate requires deterministic preparation; a malformed review queue is warning-only; malformed or invalid Earnings staging leaves both candidate and canonical dashboard unchanged. `earnings_week.js` never edits dashboard HTML, and the published dashboard must not fetch staging files at runtime.
...
There is one editorial owner: `generated/editorial/dashboard-data.json`. Deterministic preparation may carry forward commentary whose bound revision or Earnings fingerprint remains valid, but it never accepts editorial copy from deterministic staging. Finalization applies valid explicit handoff rewrites even when the underlying revision is unchanged; fingerprints and revisions control review requirements, not whether submitted editorial work is accepted.
