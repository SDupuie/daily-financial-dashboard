...
- A stale candidate/base-edition binding is rejected as an editorial input; finalization retains the validated canonical base, records the system fallback, and retries deterministic preparation rather than blocking publication.
...
- Pre-event editorial commentary remains valid through `released_awaiting_close`. The required close response—not the intermediate arrival of an actual—invalidates that preview and triggers post-event editorial replacement. A genuinely unresolvable reaction may invalidate the preview when the workflow can no longer reach `close_available`. Finalization converts unattempted narrative work into a recorded retryable unavailable disposition; it never stops publication for missing editorial evidence.
...
- News collection coverage metadata is updater-derived from the finalized qualifying story collections; partial coverage relaxes only minimum cardinality and remains retryable on later runs. It is audit metadata only and must never be rendered inline or in the footer.
